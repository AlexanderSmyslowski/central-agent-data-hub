...
For this preview, the repository checkout is the installation unit: clone is
installation, and `git pull` plus `pip install -e .` is the upgrade path. The
CLI is installed from the checkout, and operational startup/doctor/smoke paths
remain repo-local Bash scripts. See [`installation.md`](installation.md).
