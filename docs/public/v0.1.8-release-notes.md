...
connection wizard: choose the agent, follow one setup card, then test whether
...
- The agent-context page now starts the setup area with an **Agent connection
  wizard**.
- The wizard shows three explicit steps:
  - choose the agent
  - connect once where possible
  - test the handoff
- The page keeps the honest boundary: Hub View creates setup material and
  context packs; it never runs an agent by itself.
...
- A new **Test the handoff** block explains what the user should look for after
  setup.
- Chatbots should receive the copied context pack before the task.
...
A short first-run observation should confirm that a tester can find **Agent
connection wizard**, use **Choose your agent**, understand **Connection
verification**, and explain **Test the handoff**.
