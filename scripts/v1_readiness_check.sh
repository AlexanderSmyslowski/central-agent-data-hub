...
RESTORE_DRILL="$ROOT_DIR/scripts/restore_drill.sh"
FIRST_RUN_PROTOCOL="$ROOT_DIR/docs/first-run-test-protocol.md"
...
require_text "$V1_DEFINITION" "scripts/restore_drill.sh"
require_text "$V1_DEFINITION" "docs/first-run-test-protocol.md"
...
require_text "$V1_DEFINITION" "latest timestamped local"
require_text "$V1_DEFINITION" "backup was restored and checked"
...
require_text "$RELEASE_CHECK" "run_step \"Restore drill\""
...
require_text "$RESTORE_DRILL" "scripts/db_backup.sh"
require_text "$RESTORE_DRILL" "scripts/db_verify_backup.sh"
require_text "$RESTORE_DRILL" "Restore drill: ok"
require_text "$FIRST_RUN_PROTOCOL" "human proof path that complements"
require_text "$FIRST_RUN_PROTOCOL" "without maintainer memory"
